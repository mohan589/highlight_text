...
class YourHighlightingClass
  ...
  def apply_random_colors
    result = ''
    paragraphs.each_with_index do |item, index|
      data = item.split(' ')
  ...
  def html_template(input)
    File.open("highlight_text.html", "w") { |f| f.write input }
  end
...
YourHighlightingClass.new(content, highlights).apply